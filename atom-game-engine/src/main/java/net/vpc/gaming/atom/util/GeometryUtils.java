...
public class GeometryUtils {
    ...
    public static ModelPoint nextLinePoint(ModelPoint source, double angle, double speed) {
        if (speed == 0) {
            return source;
        }
        double sx = source.getX();
        double sy = source.getY();
        double nx = Math.cos(angle) * speed + sx;
        double ny = Math.sin(angle) * speed + sy;
...
//            return newPos;
//        }else{
//            double x=newPos.getX()-rect1.getX();
//            double y=newPos.getY()-rect1.getY();
//        }
//    }
    ...
    public static double[] computePointOnLine(double x0, double y0,
        ...
        double[] p = computePointOnLine(p0, p1, fractionFrom0);
    ...
    public static void extendLine(double[] p0, double[] p1, double toLength,
                                  double anchor) {
        double[] p = computePointOnLine(p0, p1, anchor);
     ...
     */
    public static void extendLine(double[] p0, double[] p1, double toLength) {
        double oldLength = getDistance(p0, p1);
    ...
    public static double getDistance(int x0, int y0, int x1, int y1) {
        return getDistance((double) x0, (double) y0,
                (double) x1, (double) y1);
    }
    ...
    public static double length(int[] x, int[] y, boolean isClosed) {
        ...
        for (int i = 0; i < nPoints - 1; i++)
            length += getDistance(x[i], y[i], x[i + 1], y[i + 1]);

        // Add last leg if this is a polygon
        if (isClosed && nPoints > 1)
            length += getDistance(x[nPoints - 1], y[nPoints - 1], x[0], y[0]);
    ...
    public static double distance(int x0, int y0, int x1, int y1,
        ...
        double length = getDistance(x0, y0, x1, y1);
        if (length == 0.0)
            return getDistance(x0, y0, x, y);
    ...
    public static double computeAngle(double[] p0, double[] p1, double[] p2) {
        ...
        double length1 = getDistance(v0);
        double length2 = getDistance(v1);
     ...
     *         0 if intesrection outside segments (x,y setMap)
     *         +1 if segments intersect (x,y setMap)
     */
    public static int findLineSegmentIntersection(double x0, double y0,
        ...
        if (equals(x0, x1)) {
            if (y0 < y1)
                distanceFrom1 = y < y0 ? getDistance(x, y, x0, y0) :
                        y > y1 ? getDistance(x, y, x1, y1) : 0.0;
            else
                distanceFrom1 = y < y1 ? getDistance(x, y, x1, y1) :
                        y > y0 ? getDistance(x, y, x0, y0) : 0.0;
        } else {
            if (x0 < x1)
                distanceFrom1 = x < x0 ? getDistance(x, y, x0, y0) :
                        x > x1 ? getDistance(x, y, x1, y1) : 0.0;
            else
                distanceFrom1 = x < x1 ? getDistance(x, y, x1, y1) :
                        x > x0 ? getDistance(x, y, x0, y0) : 0.0;
        }

        double distanceFrom2;
        if (equals(x2, x3)) {
            if (y2 < y3)
                distanceFrom2 = y < y2 ? getDistance(x, y, x2, y2) :
                        y > y3 ? getDistance(x, y, x3, y3) : 0.0;
            else
                distanceFrom2 = y < y3 ? getDistance(x, y, x3, y3) :
                        y > y2 ? getDistance(x, y, x2, y2) : 0.0;
        } else {
            if (x2 < x3)
                distanceFrom2 = x < x2 ? getDistance(x, y, x2, y2) :
                        x > x3 ? getDistance(x, y, x3, y3) : 0.0;
            else
                distanceFrom2 = x < x3 ? getDistance(x, y, x3, y3) :
                        x > x2 ? getDistance(x, y, x2, y2) : 0.0;
        }
    ...
    public static double[] findLinePolygonIntersections(double[] x, double[] y,
        ...
        for (int i = 0; i < nPoints; i++) {
            ...
            int type = findLineSegmentIntersection(x0, y0, x1, y1,
                    x2, y2, x3, y3,
                    intersection);
    ...
    public static int[] createEllipse(int[] x, int[] y) {
        ...
        double axisAngle = computeAngle(p0, p1, p2);

        // dx / dy
        double dx = getDistance(x0, y0, x[1], y[1]);
        double dy = getDistance(x0, y0, x[0], y[0]) * Math.sin(axisAngle);
        ...
        // If angle is close to 0 or 180 shear is infinite, and we setMap
        // it to zero as well.
    ...
    public static int[] createEllipse(int x0, int y0, int dx, int dy) {
        ...
        // Allocate arrays for holding the complete setMap of vertices around
        ...
        // from this first quadrant. For each angle setMap the four corresponding
        // ellipse points.
        double a = 0.0;
    ...
    public static boolean findPolygonPosition(int[] x, int[] y,
        ...
        for (int i = 1; i < x.length; i++) {
            double legLength = getDistance(x[i - 1], y[i - 1], x[i], y[i]);
    ...
    public static double getUniformAngle(double theta) {
...
//        double pi = Math.PI;
//        System.out.println(getAngleDiff(pi / 2, 0) / pi);
//    }

    public static double getAngleDiff(double fromAngle, double toAngle) {
        double pi = Math.PI;
        final double TWICE_PI = 2 * pi;
        double a = getUniformAngle(getUniformAngle(toAngle) + TWICE_PI - getUniformAngle(fromAngle));
        if (a < pi) {
            return a;
        }
        return a - TWICE_PI;
    ...
    public static String getAngleDirectionText(double lastDirection){
        if(Double.isNaN(lastDirection)){
            return "NaN";
        }
        String txt=null;
        for (Orientation orientation : Orientation.values()) {
            if(AtomUtils.safeEquals(GeometryUtils.getUniformAngle(orientation.getPhase()),GeometryUtils.getUniformAngle(lastDirection),1E-3)){
                txt=orientation.name();
            }
        }
        if(txt==null){
            txt=String.valueOf(lastDirection / Math.PI)+"PI";
        }
        return txt;
    }
}
