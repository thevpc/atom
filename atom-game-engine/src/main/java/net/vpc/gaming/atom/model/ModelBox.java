...
public class ModelBox implements Serializable {
    ...
    public Point.Double[] getAWTPointsDouble() {
        double minX = getMinX();
        double maxX = getMaxX();
        double minY = getMinY();
        double maxY = getMaxY();
        return new Point.Double[]{
                new Point.Double(minX, minY),
                new Point.Double(maxX, minY),
                new Point.Double(maxX, maxY),
                new Point.Double(minX, maxY),};
    }

    public Point.Float[] getAWTPointsFloat() {
        float minX = (float) getMinX();
        float maxX = (float) getMaxX();
        float minY = (float) getMinY();
        float maxY = (float) getMaxY();
        return new Point.Float[]{
                new Point.Float(minX, minY),
                new Point.Float(maxX, minY),
                new Point.Float(maxX, maxY),
                new Point.Float(minX, maxY),};
    }
...
//    }
    ...
    public ModelBox getDimensionBox() {
        return new ModelBox(0, 0, width, height);
    }
