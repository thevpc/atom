...
import tn.edu.eniso.soussecraft.main.business.tasks.BuildTask;
import tn.edu.eniso.soussecraft.main.dal.DALServer;
import tn.edu.eniso.soussecraft.main.dal.DALServerListener;
...
//        addSprite(placer, player, new Worker(),6,10);
        ...
        if (sprite == null) {
            for (Sprite s : selection.getSelectedSprites()) {
                if (s instanceof Worker) {
                    SpriteTask spriteTask = s.getTask();
                    if (spriteTask instanceof BuildTask) {
                        BuildTask tt = (BuildTask) spriteTask;
    ...
    public void moveSelectionToSprite(int playerId, ModelPoint point, Integer spriteId) {
        ...
        if (sprite.getPlayerId() == Player.NO_PLAYER) {
            if (sprite instanceof Resource) {
                ...
                for (Sprite sprite2 : selection.getSelectedSprites()) {
                    if (sprite2.isOwnedByPlayer(playerId)) {
                        sprite2.setTask(new GatherResourceSpriteTask(commandCenter.getId(), sprite.getId()));
                    }
                }
                return;
            }
            ...
            for (Sprite sprite2 : selection.getSelectedSprites()) {
                if (sprite2.isOwnedByPlayer(playerId)) {
                    sprite2.setTask(new AttackSpriteTask(ennemi, 2));
                }
            }
...
//                    if (sprite2.isOwnedByPlayer(playerId)) {
//                        sprite2.setSpriteTask(new AttackSpriteTask(point));
//                    }
//                }
            return;
        }
    ...
    public void moveSelectionToTile(int playerId, ModelPoint point, Integer tileId) {
        ...
        for (Sprite sprite2 : selection.getSelectedSprites()) {
            if (sprite2.isOwnedByPlayer(playerId)) {
                sprite2.setTask(new FindPathToPointSpriteTask(point));
